...
async function getDashboardStats({ date, userId, userRole }) {
  ...
  const [
    ...
    pool.query(`
      SELECT periodo, tipo, SUM(total_cantidad) AS total_cantidad, MIN(primera_vez) AS primera_vez FROM (
        SELECT HOUR(${TZ_ADJUST}) AS periodo, m.tipo, COUNT(DISTINCT m.id) AS total_cantidad, MIN(m.fecha_movimiento) AS primera_vez
        FROM movimientos m 
    ...
    pool.query(`
      SELECT tipo, SUM(total) AS total FROM (
        SELECT m.tipo, COUNT(DISTINCT m.id) as total FROM movimientos m
        LEFT JOIN usuarios u ON m.usuario_id = u.id
